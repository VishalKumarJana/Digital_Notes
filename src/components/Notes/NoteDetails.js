...
const NoteDetails = () => {
  ...
  const fetchNoteDetails = useCallback(async () => {
    ...
    try {
      ...
      const foundNote = response.data.find((n) => n.noteId.toString() === noteId);
      if (foundNote) {
  ...
  const fetchAuditLogs = useCallback(async () => {
    try {
      const response = await api.get(`/audit/note/${noteId}`);
  ...
  }, [noteId, isAdmin, fetchAuditLogs, fetchNoteDetails]);
  ...
  return (
    <div className=" min-h-[calc(100vh-74px)] md:px-10 md:py-8 sm:px-6 py-4 px-4">
      ...
      <Modals open={modalOpen} setOpen={setModalOpen} noteId={noteId} />
    </div>
  );
};
